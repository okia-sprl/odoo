...
class PurchaseOrder(models.Model):
    ...
    def _prepare_sale_order_data(
        ...
        representative = self.env['res.partner'].sudo().search([
            ('company_id', '=', dest_company.id),
            ('represent_company_id', '=', self.company_id.id)
        ], limit=1)
    ...
    def _prepare_sale_order_line_data(
        ...
        return super(
            PurchaseOrder, self.with_context(disable_change_uom_so=True)
