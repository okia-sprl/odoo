...
class MarketOrder(models.Model):
    ...
    market_product_list_id = fields.Many2one('market.product.list', 'Product list', required=True)
    ...
    def default_get(self, fields_list):
        ...
        if not company.market_supplier_id:
            raise UserError(_('Please set the market supplier ' 'in the configure before continue.'))
        result['partner_id'] = company.market_supplier_id.id

        if 'market_product_list_id' in fields_list:
            product_list = self.env['market.product.list'].search([('is_active', '=', True)], limit=1)
            if not product_list:
                raise UserError(_('You need to create at least one product list.'))
        ...
        if 'stored_date_planned' in fields_list:
            ...
            result['stored_date_planned'] = fields.Datetime.to_string(date_tomorrow)
    ...
    def _check_stored_date_planned(self):
        for order in self:
            ...
            order.order_line.write({'stored_date_planned': order.stored_date_planned})
    ...
    def onchange_market_product_list_id(self):
        ...
        for product in self.market_product_list_id.product_ids:
            if product.id in current_product_qty:
                ...
                product_qty = 0
            line = self.order_line.new({'product_id': product.id})
    ...
    def button_confirm(self):
        ...
        empty_lines = self.order_line.filtered(lambda line: not line.product_qty)
        empty_lines.unlink()

        if not self.order_line:
            raise UserError(_('Please set a quantity for at least one product'))
    ...
    def create_supplier_sale_order(self):
        ...
        if not market_buyer_id:
            raise UserError(_('You need to configure the market ' 'buyer in the configuration'))

        market_vendor_id = self.env['ir.config_parameter'].get_param('market.market_vendor_id')
        if not market_vendor_id:
            raise UserError(_('Please define the vendor for sale order' ' in the configuration'))
        default_vendor = self.env['res.users'].sudo().browse(int(market_vendor_id))

        if default_vendor.company_id != market_buyer_id.company_id:
            raise UserError(_('The company of the buyer must ' 'the same than the company of the vendor'))

        warehouse = (
            self.env['stock.warehouse'].sudo().search([('company_id', '=', default_vendor.company_id.id)], limit=1)
        )

        company_user = self.env['res.users'].sudo().search([('company_id', '=', default_vendor.company_id.id)], limit=1)
        if not company_user:
            raise UserError(_('No user found with the company %s') % default_vendor.company_id.name)
        ...
        sol_vals = sale_order_line_obj.sudo(company_user).default_get(sol_fields)
        ...
        for line in self.order_line:
            ...
            so_line.write({'product_uom': line.product_uom.id, 'product_uom_qty': line.product_qty})

        sale_order.action_confirm()
        sale_order.mapped('picking_ids').write({'min_date': self.stored_date_planned})

    def print_market_order(self):
        self.ensure_one()

        return self.env['report'].get_action(self.purchase_order_id, 'purchase.report_purchasequotation')
